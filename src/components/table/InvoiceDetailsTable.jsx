...
const InvoiceDetailsTable = ({invoiceData}) => {
	...
	const serviceOneTotalUsd = () => {
		let total = 0;
		if (invoiceData.service1.transactions[0].volume !== null){
		...
		if (invoiceData.service1.transactions[0].volume !== null){
			invoiceData.service1.transactions.map((transaction)=> {
				if (transaction.totalfee_ngn){
					...
					total = total + (transaction.totalfee_usd * invoiceData.rate)
				}
			})
		}
	...
	const serviceTwoTotalUsd = () => {
		let total = 0;
		if (invoiceData.service2.transactions[0].volume !== null){
	...
	return(
		<InvoiceContainer>
			...
			{
				invoiceData.service1.transactions[0].volume !== null && <div>
					...
					<Table>
						...
						<TableRow>
							...
							<TableHeader>Unit Fee ({invoiceData.currency || "NGN"})</TableHeader>
							<TableHeader>Total Fee ({invoiceData.currency || "NGN"})</TableHeader>
							{invoiceData.currency !== "NGN" && <TableHeader>Total Fee (NGN)</TableHeader>}
						...
						<tbody>
						{
							invoiceData.service1.transactions.map((transaction, index) => <TableRow key={index}>
								...
								{invoiceData.currency !== "NGN" && <TableData style={{textAlign: "right"}}>{transaction.totalfee_ngn ? formatNumber(transaction.totalfee_ngn) : formatNumber(transaction.totalfee_usd * invoiceData.rate)}</TableData>}
							</TableRow>)
						}
			...
			{
				invoiceData.service2.transactions[0].volume !== null && <div>
					<a href="#" className="text-right justify-end flex text-blue-500">+ Add Transaction</a>
					...
					<Table>
						...
						<tbody>
						{
							invoiceData.service2.transactions.map((transaction, index) => <TableRow key={index}>
								...
								<TableData>{formatNumber(transaction.totalfee_usd)}</TableData>
								<TableData
									style={{textAlign: "right"}}>{transaction.totalfee_ngn ? formatNumber(formatNumber(transaction.totalfee_ngn)) : formatNumber((transaction.totalfee_usd * invoiceData.rate))}</TableData>
							</TableRow>)
						}
						<TableRow style={{fontWeight: 500}}>
							<TableData colSpan="3">Sub-Total</TableData>
			...
			<Table>
				<TableRow style={{fontWeight: 500}}>
					...
					<TableData
						style={{width: "22%"}}>{formatNumber((serviceOneTotalUsd() + serviceTwoTotalUsd()))}</TableData>
					<TableData
						style={{textAlign: "right"}}>{formatNumber((serviceOneTotalNgn() + serviceTwoTotalNgn()))}</TableData>
				</TableRow>
				<TableRow style={{fontWeight: 500}}>
					<TableData colSpan="5">VAT</TableData>
					<TableData>{formatNumber((7.5 / 100) * (serviceOneTotalUsd() + serviceTwoTotalUsd()))}</TableData>
					<TableData
						style={{textAlign: "right"}}>{formatNumber((invoiceData.vat / 100) * (serviceOneTotalNgn() + serviceTwoTotalNgn()))}</TableData>
				</TableRow>
				<tfoot>
				<TableRow style={{fontWeight: 500}}>
					<TableData colSpan="5" style={{padding: "8px"}}>GRAND TOTAL in USD</TableData>
					<TableData style={{textAlign: "right"}}
					           colSpan="5">{formatNumber(((invoiceData.vat / 100) * (serviceOneTotalUsd() + serviceTwoTotalUsd()) + (serviceOneTotalUsd() + serviceTwoTotalUsd())))}</TableData>
				</TableRow>
				<TableRow style={{fontWeight: 500}}>
					<TableData colSpan="5">GRAND TOTAL in NGN (CBN: N{invoiceData.rate}/$1)</TableData>
					<TableData style={{textAlign: "right"}}
					           colSpan="5">{formatNumber(((invoiceData.vat / 100) * (serviceOneTotalNgn() + serviceTwoTotalNgn()) + (serviceOneTotalNgn() + serviceTwoTotalNgn())))}</TableData>
				</TableRow>
				</tfoot>
